...
public class AddNewSts extends Frame implements ActionListener
{
Connection con;
ResultSet rec;
Statement st;
...
Frame frm;
 ...
 frm=new Frame("Add Student Detail:");
 }
public void setUpAddNewSts()
 {
 frm.setLayout(null);
 frm.setSize(700,700);
 lbler.setBounds(200,450,200,25);
...
rec=st.executeQuery("select * from AddnewSts where rollno='"+str+"'");
if(rec.next())
{
lbler.setText("RollNo alredy Exit!");
}
else
{
putData();
...
try
{
pst=con.prepareStatement("insert into addNewSts values('"+a1+"','"+a2+"','"+a3+"','"+a4+"','"+a5+"','"+a6+"','"+a7+"','"+a8+"','"+a9+"','"+a10+"')");
pst.executeUpdate();
}
